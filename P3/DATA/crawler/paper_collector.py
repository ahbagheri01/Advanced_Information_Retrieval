    ...
    def collect(self):
        ...
        for subject in self.subjects:
            ...
            for i in range(2):
                ...
                print(f"{subject} len in this query = {len(papers)} len all = {size_all + len(papers_dict)}")
                print(f"{subject} sum = {len(papers)} ")
                path = os.path.join(os.getcwd(), './data')
                Path(path).mkdir(parents=True, exist_ok=True)
                print(f"saving in {os.path.join(path, f'{subject}.json')}")
                with open(os.path.join(path, f'{subject}.json'), 'w') as f:
...
if __name__ == "__main__":
    lim = 2
    n = int(10000/lim)-1
    data_collector = DataCollector(['Language Models','NLP'], n = n , lim = lim)
